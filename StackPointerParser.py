...
def main():
    ...
    args_parser.add_argument('--num_epochs', type=int, default=30, help='Number of training epochs')
    ...
    args_parser.add_argument('--decoder_layers', type=int, default=2, help='Number of layers of decoder RNN')
    args_parser.add_argument('--char_num_filters', type=int, default=300, help='Number of filters in CNN(Character Level)')
    args_parser.add_argument('--char_window_size',type=int,default=3,help='Size of window in CNN(Character Level)')
    args_parser.add_argument('--eojul_num_filters', type=int, default=300, help='Number of filters in CNN(Eojul Level)')
    args_parser.add_argument('--eojul_window_size',type=int, default=3, help='Size of window in CNN(Eojul Level)')
    ...
    args_parser.add_argument('--gamma', type=float, default=1e-5, help='weight for regularization')
    ...
    args_parser.add_argument('--n_head', type=int, default=8, help='number of multi-head')
    ...
    args_parser.add_argument('--use_kkma',action="store_true",help="use kkma")
    args_parser.add_argument('--end_to_end',action="store_true",help="end_to_end")
    ...
    char_window = args.char_window_size
    eojul_num_filters = args.eojul_num_filters
    eojul_window = args.eojul_window_size
    ...
    logger.info("Embedding dim: word=%d (%s), char=%d (%s), pos=%d (%s)" % (word_dim[0], word_status, char_dim, char_status, pos_dim, pos_status))
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in range(1, num_batches + 1):
            ...
            input_encoder, input_decoder = conllx_stacked_data.get_batch_stacked_variable(data_train, batch_size, unk_replace=unk_replace, use_gpu=use_gpu,end_to_end=end_to_end)
        ...
        dev_total_root = 0.0
        dev_total_inst = 0.0
        for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_dev, batch_size, use_gpu=use_gpu,end_to_end=end_to_end):
            ...
            if end_to_end:
                word,pre_word,post_word,post_bi_word,pre_bi_word = word
                word = word.data.cpu().numpy()
                pre_word = pre_word.data.cpu().numpy()
                post_word = post_word.data.cpu().numpy()
                post_bi_word = post_bi_word.data.cpu().numpy()
                pre_bi_word = pre_bi_word.data.cpu().numpy()
            else:
                word = word.data.cpu().numpy()
            ...
            stats, stats_nopunc, stats_root, num_inst = parser.eval(word, pos, heads_pred, types_pred, heads, types, word_alphabet, pos_alphabet, lengths, punct_set=punct_set, symbolic_root=True,end_to_end=end_to_end)
        ...
        if dev_lcorrect_nopunc < dev_lcorr_nopunc or (dev_lcorrect_nopunc == dev_lcorr_nopunc and dev_ucorrect_nopunc < dev_ucorr_nopunc):
            ...
            test_root_correct = 0.0
            test_total_root = 0
            for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_test, batch_size, use_gpu=use_gpu,end_to_end=end_to_end):
                input_encoder, _, sentences = batch
                word, char, pos, heads, types, masks, lengths = input_encoder
                heads_pred, types_pred, _, _ = network.decode(word, char, pos, mask=masks, length=lengths, beam=beam, leading_symbolic=conllx_stacked_data.NUM_SYMBOLIC_TAGS)
