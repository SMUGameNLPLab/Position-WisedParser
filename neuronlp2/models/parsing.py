...
class StackPtrNet(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, char_num_filters, char_kernel_size,
                 ...
                 biaffine=True, pos=True, char=True, eojul=True, prior_order='inside_out', skipConnect=False, grandPar=False, sibling=False,n_head=8,end_to_end=False):

        super(StackPtrNet, self).__init__()
        if end_to_end:
            word_dim,pre_dim,post_dim,post_bi_dim,pre_bi_dim = word_dim
            num_words,num_pre_words,num_post_words,num_post_bi_words,num_pre_bi_words = num_words
            embedd_word,embedd_pre_word,embedd_post_word,embedd_post_bi_word,embedd_pre_bi_word = embedd_word

            self.word_embedd = Embedding(num_words, word_dim, init_embedding=embedd_word)
            self.pre_word_embedd = Embedding(num_pre_words,pre_dim,init_embedding=embedd_pre_word)
            self.post_word_embedd = Embedding(num_post_words, post_dim, init_embedding=embedd_post_word)
            self.post_bi_word_embedd = Embedding(num_post_bi_words,post_bi_dim,init_embedding=embedd_post_bi_word)
            self.pre_bi_word_embedd = Embedding(num_pre_bi_words,pre_bi_dim,init_embedding=embedd_pre_bi_word)
        else:
            self.word_embedd = Embedding(num_words, word_dim, init_embedding=embedd_word)

        self.pos_embedd = Embedding(num_pos, pos_dim, init_embedding=embedd_pos) if pos else None
        self.char_embedd = Embedding(num_chars, char_dim, init_embedding=embedd_char)
        self.char_conv1d = nn.Conv1d(char_dim, char_num_filters, char_kernel_size, padding=char_kernel_size - 1)

        self.eojul_conv1d = nn.Conv2d(6,1,(1,1))
    ...
    def _get_encoder_output(self, input_word, input_char, input_pos, mask_e=None, length_e=None, hx=None):
        ...
        if self.end_to_end:
            ...
            pre_word = self.pre_word_embedd(input_word[1])
            post_word = self.post_word_embedd(input_word[2])
            post_bi_word = self.post_bi_word_embedd(input_word[3])
            pre_bi_word = self.pre_bi_word_embedd(input_word[4])

            word = self.dropout_in(word)
            pre_word = self.dropout_in(pre_word)
            post_word = self.dropout_in(post_word)
            post_bi_word = self.dropout_in(post_bi_word)
            pre_bi_word = self.dropout_in(pre_bi_word)
            ...
            word = torch.cat([word.unsqueeze(1),pre_word.unsqueeze(1),post_word.unsqueeze(1),post_bi_word.unsqueeze(1),pre_bi_word.unsqueeze(1),char.unsqueeze(1)],dim=1)
